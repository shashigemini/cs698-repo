...
class TestTokenCleanupTask:
    ...
    async def test_token_cleanup_run(self, test_settings):
        with patch("app.dependencies._database") as mock_db:
            ...
            with patch.object(
                RevokedTokenRepository, "cleanup_expired", new_callable=AsyncMock
            ) as mock_cleanup:
    ...
    async def test_start_stop(self):
        ...
        with patch.object(
            cleanup_task, "_run_loop", new_callable=AsyncMock
        ) as mock_loop:
            cleanup_task.start()
...
class TestAuthServiceAdditional:
    ...
    async def test_logout(self, test_settings):
        ...
        await auth.logout(
            user_id=user_id,
            token_jti="test-jti",
            token_exp=int(datetime.now().timestamp()) + 3600,
        )
    ...
    async def test_refresh_tokens_failure(self, test_settings):
        ...
        auth._token_service.validate_refresh_token = AsyncMock(
            side_effect=RefreshTokenError("Invalid")
        )
        auth._token_service.rotate_refresh_token = AsyncMock(
            side_effect=RefreshTokenError("Invalid")
        )
    ...
    async def test_register_service(self, test_settings):
        ...
        auth._token_service = MagicMock()
        auth._token_service.generate_token_pair = MagicMock(
            return_value={
                "access_token": "a",
                "refresh_token": "r",
    ...
    async def test_recover_account_success(self, test_settings):
        ...
        await auth.recover_account(
            email="user@test.com", new_auth_token="new", new_wrapped_account_key="wak"
        )
    ...
    async def test_refresh_tokens_success(self, test_settings):
        ...
        mock_user.role = "user"
        auth._user_repo = AsyncMock()
        auth._user_repo.get_by_id = AsyncMock(return_value=mock_user)
        auth._token_service.rotate_refresh_token = AsyncMock(
            return_value={
                "access_token": "a",
                "refresh_token": "r",
...
class TestDocumentServiceAdditional:
    ...
    async def test_full_process_document_pipeline_success(self, test_settings):
        ...
        with (
            patch("app.dependencies._database") as mock_db,
            ...
            with patch.object(
                DocumentRepository, "update_status", new_callable=AsyncMock
            ) as mock_update:
                await svc._process_document_pipeline(
                    doc_id=uuid.uuid4(), file_path="/tmp/test.pdf", title="Test Doc"
                )
