...
# Mock models
class MockUser:
    def __init__(self, id=None, email="test@example.com", password_hash="hash", role="user"):
...
class MockE2EEKeys:
    def __init__(self, user_id=None, salt=b"salt", wrapped_account_key="wak", recovery_wrapped_ak="rwak"):
...
async def test_register_standard_base64_salt_auth_04(auth_service):
    ...
    auth_service._token_service.generate_token_pair.return_value = {"access_token": "at", "refresh_token": "rt", "access_expires_at": datetime.now(timezone.utc)}
...
async def test_login_verify_success_auth_11(auth_service):
    ...
    result = await auth_service.login_verify(email="user@example.com", client_auth_token="secret-token")
...
async def test_login_verify_ghost_user_auth_12(auth_service):
    ...
    with pytest.raises(AuthError):
        await auth_service.login_verify(email="ghost@example.com", client_auth_token="any")
...
async def test_login_verify_wrong_token_auth_13(auth_service):
    ...
    with pytest.raises(AuthError):
        await auth_service.login_verify(email="user@example.com", client_auth_token="wrong-token")
...
async def test_login_verify_keys_missing_auth_14(auth_service):
    ...
    with pytest.raises(KeysNotFoundError):
        await auth_service.login_verify(email="user@example.com", client_auth_token="token")
