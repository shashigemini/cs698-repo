...
*   **OpenAI**: Used for embeddings and answer generation in the RAG flow.
*   **Qdrant-Client**: Python driver for the Qdrant vector database.
*   **argon2-cffi & python-jose**: For secure password hashing and JWT handling.
...
2. **Start the development environment:**
   ```bash
   docker compose -f dev_test/docker-compose.dev.yml up -d --build
   ```
...
3. **View application logs:**
   ```bash
   docker compose -f dev_test/docker-compose.dev.yml logs -f app
   ```

### 3. Testing
For a complete guide on our multi-layered testing strategy (Unit, Integration, E2E, and Mutation), see [docs/product/test_suite_report.md](C:/Users/shash/Documents/GitHub/cs698-repo/docs/product/test_suite_report.md).
...
```bash
docker compose -f dev_test/docker-compose.dev.yml stop
```
...
```bash
docker compose -f dev_test/docker-compose.dev.yml down -v
```
