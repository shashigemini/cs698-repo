...
Information Flows
App Startup Flow
User opens app (web/iOS/Android)
StartupScreen displays loading indicator
...
Navigate to AuthScreen (Login/Register tabs)
User enters email and password
AuthRepository calls /auth/login or /auth/register
On success:
...
ChatScreen loads conversation history (if conversation_id exists)
User types query and sends
ChatRepository calls /api/chat/query with:
Authorization header: Bearer <access_token>
conversation_id: UUID (or null for new conversation)
guest_session_id: null
HTTP Interceptor checks token expiry before request:
...
Logout Flow
User clicks logout button (in app drawer/menu)
...
ConversationHistoryScreen: (Internal) Redirects to SettingsScreen history section
SettingsScreen: User preferences, history management, and account deletion
State Diagrams
...
Ensuring guest vs auth flows work identically across web, iOS, Android
...
Handling keyboard, scroll, and back button behavior on mobile
...
Token refresh integration with app lifecycle (foreground/background)
...
Conversation and Account Management (NEW)
- `AuthRepository.deleteAccount()`: Permanent account and data removal.
- `ChatRepository.getConversations()`: Fetch history for drawer and settings.
- `ChatRepository.deleteConversation(id)`: Individual history pruning.
- `ChatRepository.exportConversation(id)`: JSON/Markdown export for user data portability.
...
Risks to Completion
UI/UX complexity around showing limits and transitions between guest/auth
...
Tight coupling to backend error codes
...
Platform-specific bugs (iOS vs Android vs Web)
...
Testing complexity (multiple platforms, screen sizes, network conditions)
...
App store compliance (iOS/Android review delays)
